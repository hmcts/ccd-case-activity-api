...
describe('Activity Service - GetActivities', () => {
  ...
  it('should retrieve all activities for a case', (done) => {
    const a1 = testUtils.addActivity(1242, 111, 'view', fName, sName);
    const a2 = testUtils.addActivity(10, 111, 'view', fName, sName);
    const a3 = testUtils.addActivity(10, 121, 'edit', fName, sName);

    Promise.all([a1, a2, a3])
      .then(() => {
        Promise.all([
          activityAssert.caseViewersEquals(111, ['1242', '10']),
          activityAssert.caseEditorsEquals(121, ['10'])]);
        chai.request(server)
          ...
          .end((err, res) => {
            ...
            res.body[0].viewers[0].forename.should.equal(fName);
            res.body[0].viewers[0].surname.should.equal(sName);
  ...
  it('should get only the non expired activities', (done) => {
    ...
    delayed(AfterActivitiesExpired, () => {
      const a4 = testUtils.addActivity(88, 111, 'view', fName, sName);
      const a5 = testUtils.addActivity(98, 111, 'view', fName, sName);
      const a6 = testUtils.addActivity(198, 121, 'edit', fName, sName);

      Promise.all([a4, a5, a6])
        .then(() => {
          Promise.all([
            activityAssert.caseViewersEquals(111, ['1242', '10', '88', '98']),
            activityAssert.caseEditorsEquals(121, ['10', '198'])]);
          chai.request(server)
            .get('/cases/111,121/activity')
            .set('Authorization', Token)
            .end((err, res) => {
              ...
              res.body[0].viewers[0].forename.should.equal(fName);
              res.body[0].viewers[1].surname.should.equal(sName);
              res.body[0].editors.should.be.a('array');
              res.body[0].editors.length.should.equal(0);
              res.body[1].editors.length.should.equal(1);
              done();
  ...
  it('should display unknown viewer/editor count for tho users whose info is expired', (done) => {
    const a1 = testUtils.addActivity(11, 111, 'view', fName, sName);
    const a2 = testUtils.addActivity(12, 121, 'edit', fName, sName);

    Promise.all([a1, a2])
      .then(() => Promise.all([
        activityAssert.caseViewersEquals(111, ['11']),
        activityAssert.caseEditorsEquals(121, ['12'])]))
      .catch(error => done(error));
    ...
    delayed(USER_EXPIRED_TIME, () => {
      ...
      Promise.all([a4, a5, a6])
        .then(() => {
          Promise.all([
            activityAssert.caseViewersEquals(111, ['11', '88', '98']),
            activityAssert.caseEditorsEquals(121, ['12', '198'])]);
          chai.request(server)
            .get('/cases/111,121/activity')
            .set('Authorization', Token)
            .end((err, res) => {
